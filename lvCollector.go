...
func newLvmLvCollector() *lvmLvCollector {
	return &lvmLvCollector{
		lvDataSizeMetric: prometheus.NewDesc("lvm_lv_size_bytes",
			...
			[]string{"lv_pool_name", "lv_name"}, nil,
		),
...
func (collector *lvmLvCollector) Collect(ch chan<- prometheus.Metric) {
	out, err := exec.Command("/sbin/lvs", "--units", "B", "--separator", ",", "-o", "lv_size,lv_metadata_size,data_percent,metadata_percent,pool_lv,lv_name", "--noheadings").Output()
	if err != nil {
		log.Print(err)
	}
	...
	for _, line := range lines {
		...
		for index, descriptor := range descriptors {
			...
			ch <- prometheus.MustNewConstMetric(descriptor, prometheus.GaugeValue, value, poolName, logicalVolumeName)
		}
	}
}
